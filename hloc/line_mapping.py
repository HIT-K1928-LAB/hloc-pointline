...
def _project_world_points(query_camera: pycolmap.Camera, cam_from_world: pycolmap.Rigid3d, points3d: np.ndarray):
    ...
    cam_xy = points_cam_valid[:, :2] / np.clip(points_cam_valid[:, 2:3], EPS, None)
    proj_valid = np.asarray(query_camera.img_from_cam(cam_xy), dtype=np.float64)
    finite_mask = np.all(np.isfinite(proj_valid), axis=1)
    if np.any(finite_mask):
        valid_indices = np.where(valid)[0][finite_mask]
        proj[valid_indices] = proj_valid[finite_mask]
    if not np.all(finite_mask):
        invalid_indices = np.where(valid)[0][~finite_mask]
        valid[invalid_indices] = False
    return proj, valid
