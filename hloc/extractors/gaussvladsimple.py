...
#     def init_params(self, clsts, traindescs):
#         self.centers.data = torch.from_numpy(clsts).T[None]
...
class DiagonalGMMPosterior(nn.Module):
    ...
    def forward(self, x):  # x: B × D × N
        ...
        # mu_expand = self.mu.unsqueeze(0).unsqueeze(0)  # 1 × 1 × K × D
        ...
        # posterior = F.softmax(self.alpha*logits, dim=-1)  # B × N × K
...
class gaussvladsimplehloc(BaseModel):
    required_inputs = ["image"]
    def _init(self,conf):
        self.model =  AttnVLAD(whiten=False)
    # default_conf = {"model_name": "VGG16-NetVLAD-Pitts30K", "whiten": True}
        weight = "/home/hxy/doctor/feature dectect/hloc/Hierarchical-Localization/hloc/extractors/weights/gaussvladsimple.pth.tar"
